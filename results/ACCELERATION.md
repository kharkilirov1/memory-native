...
| M6 | int8 Tensor-Core compute path | **done (forward path) / partial (update)** — `forward_compute="int8"` wires the int8 forward into training (`int8_forward_ternary`, per-token scale, deterministic round-to-nearest so reversible/eager stay valid; grad_x + update stay fp, straight-through). T4 end-to-end: it has a **size crossover** — net-negative at d=512 (17.6k→16.8k tok/s, quantize overhead > GEMM saving on small layers) but net-positive at d=768 (5.9k→6.1k, 0.91→0.95× dense), and ×2.05 on an isolated d=2048 forward → wins more as models grow. Opt-in (fp default), recommended at d≥768. The int8 *update* correlation now **reuses the saved int8 activation** (`update_compute="int8"` + `act_save_bits=8` → `int8_correlation_presaved`): it folds the saved per-token x scale into grad_out and quantizes only grad_out, so x is never re-quantized. T4 (`gpu_int8_presaved_update_T4.log`): **×1.45 at K=2048 (5.61 vs 8.16 ms), ×2.16 at K=8192 (12.9 vs 27.9 ms)** vs fp32 cuBLAS — the open negative is closed; the re-quant variant (8.34 / 23.5 ms) was the problem. |
...
d=2048 forward is ×2.05 — so on large models it is the path to *match or beat* dense speed. The
int8-update path (reusing the saved activation) adds **×1.45–2.16 on the grad_w correlation**
(growing with K). Net: the counter method is ~0.89–0.95× dense speed today (fp / int8 forward,
scale-dependent), and with both int8 forward + int8 presaved update the large-d GEMMs (forward and
update) are each ~×1.5–2 faster than fp — the path to **matching or beating dense at scale**, at
multiples-less memory. Both are opt-in behind parity gates; fp stays the default.
