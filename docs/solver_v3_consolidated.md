...
Ветка `kimi/solver-v3-consolidated`, база `agent/solver-v3-group-recovery` @ `ad44753`.
Поглощает `kimi/solver-v3-stage-a` (закрывается без мерджа) и делает репозиторий
